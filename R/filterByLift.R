...
filterByLift <- function(g, lift.threshold=2) {
  ...
  print(paste0('Analyzing graph by lift to remove transient edges having lift < ',lift.threshold,'...'))
        ...
        for(j in 1:length(B_and_C)) {
          ...
          if(length(C)>0) {
            for(k in 1:length(C)) {
              ...
              if(DEBUG) print(paste0('     Analyzing ',B_name,'->',C_name,' (prob=',round(p_BC*100,1),'%) (from ',A_name,' prob=',round(pACpBC*100,1),'%)...'))
              if(V(g)[C_id] %in% B_and_C) {

                p_AC=E(g)$prob[E(g)$e1_concept_id==V(g)[A_id]$concept_id & E(g)$e2_concept_id==V(g)[C_id]$concept_id]
                if(DEBUG) print(paste0('     There also exists a direct link from ',A_name,' to ',C_name,' (prob=',round(p_AC*100,1),'%)...'))
                lift=p_AC/pACpBC
                #print(paste0('     ',lift))
                if(lift<lift.threshold) {
                  print(paste0('     ',A_id,'/',length(V(g)),' REMOVE direct link from ',A_name,' to ',C_name,' (prob=',round(p_AC*100,1),'%) as there is also a link through ',B_name,' (prob=',round(pACpBC*100,1),'%) (lift=',round(lift,1),')...'))
