...
PlotTrajectoriesGraphForEvent<-function(connection,
  ...
  if(!eventId %in% igraph::V(g)$concept_id) {
    msg=paste0('Event with concept_id=',eventId,' is missing from TrajectoriesGraph object. The plot for this is not created (skipping).')
  ...
  ParallelLogger::logInfo("Creating graphs for concept_id=",eventId," ('",EVENTNAME,"')...")
  ...
  if(limitOfNodes==F) {
    ...
    filename_template=paste0(ifelse(stringi::stri_length(EVENTNAME)<=20,EVENTNAME,paste(substr(EVENTNAME,1,20))), eventId, '.constructed') #add event ID to file name as the beginning of the concept name might not be unique
  } else {
    ...
    filename_template=paste0(ifelse(stringi::stri_length(EVENTNAME)<=20,EVENTNAME,paste(substr(EVENTNAME,1,20))), eventId, ".constructed.limit",limitOfNodes," events") #add event ID to file name as the beginning of the concept name might not be unique
  }
  ParallelLogger::logInfo(" Step 1: Constructing filtered graph for event ",eventId,": '",title,"'...")
  ...
  ParallelLogger::logInfo(' ...done. File saved to ',filename,'.')


  ParallelLogger::logInfo(' Step 2: Aligning actual patient trajectories to this graph...')
  ...
  if(skipOutputTables==F) {
    ...
    ParallelLogger::logInfo(' Step 3: Saving graph data to edge/vertex files: ',filename_e,' and ',filename_v,'...')
    ...
    ParallelLogger::logInfo(' Step 3: (skipping as skipOutputTables=T)')
  }

  ParallelLogger::logInfo(' Step 4: Printing aligned graph: ')
