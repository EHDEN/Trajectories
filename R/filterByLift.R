...
filterByLift <- function(g, lift.threshold=2) {
  ...
  ParallelLogger::logInfo('Analyzing graph by lift to remove transient edges having lift < ',lift.threshold,'...')
  ...
  while(EDGE_REMOVED==T) {
    ...
    for(A_id in (last_A_id+1):length(V(g))) {
      ...
      if(DEBUG) ParallelLogger::logDebug(paste0('Analyzing ',A_name,'...'))
      #all outgoing edges from initial node A
      edges_from_A <- incident(g, v = V(g)[A_id], mode = c("out"))
      B_and_C<-head_of(g, edges_from_A)
      if(length(B_and_C)>0) {
        if(DEBUG) ParallelLogger::logDebug(paste0('Its descendants: ',paste(V(g)[B_and_C]$name, collapse=", ")))
        for(j in 1:length(B_and_C)) {
          #print(j)
          B_name=B_and_C[j]$name
          B_id<-which(V(g)$name==B_name)
          p_AB=edges_from_A[j]$prob
          if(DEBUG) ParallelLogger::logDebug(paste0('  Analyzing ',A_name,'->',B_name,' (prob=',round(p_AB*100,1),'%)...'))
          ...
          if(length(C)>0) {
            for(k in 1:length(C)) {
              ...
              if(DEBUG) ParallelLogger::logDebug(paste0('     Analyzing ',B_name,'->',C_name,' (prob=',round(p_BC*100,1),'%) (from ',A_name,' prob=',round(pACpBC*100,1),'%)...'))
              if(V(g)[C_id] %in% B_and_C) {

                p_AC=E(g)$prob[E(g)$e1_concept_id==V(g)[A_id]$concept_id & E(g)$e2_concept_id==V(g)[C_id]$concept_id]
                if(DEBUG) ParallelLogger::logDebug(paste0('     There also exists a direct link from ',A_name,' to ',C_name,' (prob=',round(p_AC*100,1),'%)...'))
                lift=p_AC/pACpBC
                #print(paste0('     ',lift))
                if(lift<lift.threshold) {
                  ParallelLogger::logInfo(paste0('     ',A_id,'/',length(V(g)),' REMOVE direct link from ',A_name,' to ',C_name,' (prob=',round(p_AC*100,1),'%) as there is also a link through ',B_name,' (prob=',round(pACpBC*100,1),'%) (lift=',round(lift,1),')...'))
  ...
  ParallelLogger::logInfo(num_edges_removed,' edges removed from the graph by lift analysis, ',length(E(g)),' edges remaining.')
  return(g)
}
