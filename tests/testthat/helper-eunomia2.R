...
addRandomEventForPeson<-function(connection,person_id=6,exclude_concept_ids=c(),include_concept_ids=c()) {
  ...
  executeSql(connection, sql=sql, progressBar = F,
             reportOverallTime=F)
  #print(paste0('Added ',concept_id,' for person ',person_id))
...
limitToNumPatients<-function(connection,n=2694) {
  ...
  executeSql(connection, paste0("DELETE FROM CONDITION_OCCURRENCE WHERE person_id NOT IN (",paste(person_ids,collapse=","),");"), progressBar = F,
             reportOverallTime=F)
  print(paste0('Number of patients limited to ',n,' in the database'))
}
...
#duplicates persons, observation_periods, conditions, returns mapping table between old and duplicated person id-s
duplicatePersonsAndTheirConditions<-function(connection) {

  #duplicate persons to temp table
  executeSql(connection, paste0("DROP TABLE IF EXISTS person_dup;"), progressBar = F,
             reportOverallTime=F)
  executeSql(connection, paste0('CREATE TABLE person_dup AS
SELECT * FROM person;'), progressBar = F,
             reportOverallTime=F)
  res<-querySql(connection, paste0('SELECT MAX(person_id) AS MAX_PERSON_ID FROM person;'))
  max_person_id<-res$MAX_PERSON_ID

  #duplicate observation_period to temp table
  executeSql(connection, paste0("DROP TABLE IF EXISTS OBSERVATION_PERIOD_DUP;"), progressBar = F,
             reportOverallTime=F)
  executeSql(connection, paste0('CREATE TABLE OBSERVATION_PERIOD_DUP AS
SELECT * FROM OBSERVATION_PERIOD;'), progressBar = F,
             reportOverallTime=F)

  #duplicate conditions to temp table
  executeSql(connection, paste0("DROP TABLE IF EXISTS CONDITION_OCCURRENCE_DUP;"), progressBar = F,
             reportOverallTime=F)
  executeSql(connection, paste0('CREATE TABLE CONDITION_OCCURRENCE_DUP AS
SELECT * FROM CONDITION_OCCURRENCE;'), progressBar = F,
             reportOverallTime=F)
  #res<-querySql(connection, paste0('SELECT MAX(CONDITION_OCCURRENCE_ID) AS MAX_CONDITION_OCCURRENCE_ID FROM CONDITION_OCCURRENCE;'))
  #max_condition_id<-res$MAX_CONDITION_OCCURRENCE_ID
  #querySql(connection, 'SELECT * FROM CONDITION_OCCURRENCE_DUP LIMIT 5;')

  #create new person_ids
  executeSql(connection, paste0('ALTER TABLE person_dup ADD person_id_old INT;'), progressBar = F,
             reportOverallTime=F)
  executeSql(connection, paste0('UPDATE person_dup SET person_id_old=person_id;'), progressBar = F,
             reportOverallTime=F)
  executeSql(connection, paste0('UPDATE person_dup SET person_id=person_id_old+',max_person_id), progressBar = F,
             reportOverallTime=F)

  #update person_ids and observation_period_ids in OBSERVATION_PERIOD_DUP
  executeSql(connection, paste0('UPDATE OBSERVATION_PERIOD_DUP SET person_id=
                                (SELECT person_id FROM person_dup WHERE person_id_old=OBSERVATION_PERIOD_DUP.person_id);'), progressBar = F,
             reportOverallTime=F)
  executeSql(connection, paste0('UPDATE OBSERVATION_PERIOD_DUP SET OBSERVATION_PERIOD_ID=PERSON_ID;'), progressBar = F,
             reportOverallTime=F)
  #querySql(connection, 'SELECT * FROM OBSERVATION_PERIOD_DUP LIMIT 5;')

  #update person_ids in CONDITION_OCCURRENCE_DUP
  executeSql(connection, paste0('UPDATE CONDITION_OCCURRENCE_DUP SET person_id=
                                (SELECT person_id FROM person_dup WHERE person_id_old=CONDITION_OCCURRENCE_DUP.person_id);'), progressBar = F,
             reportOverallTime=F)
  #querySql(connection, 'SELECT * FROM CONDITION_OCCURRENCE_DUP LIMIT 5;')

  #get person_id mapping table
  res=querySql(connection, paste0('SELECT person_id AS PERSON_ID_NEW,person_id_old AS PERSON_ID_OLD FROM person_dup ORDER BY person_id'))

  #remove "person_id_old" column from person_dup
  executeSql(connection, paste0('ALTER TABLE person_dup DROP person_id_old;'), progressBar = F,
             reportOverallTime=F)

  #add person_dup data to person table
  executeSql(connection, paste0('INSERT INTO person SELECT * FROM person_dup;'), progressBar = F,
             reportOverallTime=F)

  #add OBSERVATION_PERIOD_DUP data to OBSERVATION_PERIOD table
  executeSql(connection, paste0('INSERT INTO OBSERVATION_PERIOD SELECT * FROM OBSERVATION_PERIOD_DUP;'), progressBar = F,
             reportOverallTime=F)

  #add CONDITION_OCCURRENCE_DUP data to CONDITION_OCCURRENCE table
  executeSql(connection, paste0('INSERT INTO CONDITION_OCCURRENCE SELECT * FROM CONDITION_OCCURRENCE_DUP;'), progressBar = F,
             reportOverallTime=F)
...
replaceEventWithRandomEventForPeson<-function(connection,person_id=6,concept_id_to_replace,exclude_concept_ids=c(),include_concept_ids=c()) {
  ...
  sql=paste0("UPDATE CONDITION_OCCURRENCE SET condition_concept_id=",concept_id," WHERE person_id=",person_id," AND condition_concept_id=",concept_id_to_replace,";")
