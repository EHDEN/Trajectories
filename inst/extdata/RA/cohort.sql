...
WHERE C.observation_date >= DATEFROMPARTS(2010, 01, 01)
AND C.ordinal = 1
...
WHERE C.drug_exposure_start_date >= DATEFROMPARTS(2010, 01, 01)
AND C.ordinal = 1
-- End Drug Exposure Criteria

  ) E
  JOIN @cdm_database_schema.observation_period OP on E.person_id = OP.person_id and E.start_date >=  OP.observation_period_start_date and E.start_date <= op.observation_period_end_date
  WHERE DATEADD(day,365,OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,548,E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
...
select event_id, person_id, 
  case when DATEADD(day,1826,start_date) > start_date then DATEADD(day,1826,start_date) else start_date end as end_date
INTO #strategy_ends
